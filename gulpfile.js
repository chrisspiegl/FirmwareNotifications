...
const paths = {
  ...
  views: ['views/**/*'],
  sass: ['frontend/**/*.scss', 'ftonend/**/*.sass'],
  js: [
    // 'frontend/vendors/jquery-3.2.1.min.js',
    // 'frontend/vendors/chart-2.7.2.bundle.min.js',
    'frontend/script.js',
  ],
...
gulp.task('localtunnelWebserver', (cb) => {
  ...
  var opts = {
    subdomain: 'spieglio-fwn-bw2siafw63'
  }
...
gulp.task('startCrawler', (cb) => {
  ...
  stream.on('start', () => {
    console.log('nodemon crawler => started')
  })
    .on('quit', () => cb())
    .on('restart', function () {
      console.log('nodemon crawler => restart')
    })
    .on('crash', function () {
      console.error('nodemon crawler => app crasehd\n')
      // stream.emit('restart', 10)  // restart the server in 10 seconds
    })
})

gulp.task('startListener', function (cb) {
  const stream = nodemon({
  ...
  stream.on('start', () => {
    console.log('nodemon botListener => started')
  })
    .on('quit', () => cb())
    .on('restart', function () {
      console.log('nodemon botListener => restart')
    })
    .on('crash', function () {
      console.error('nodemon botListener => app crasehd\n')
      // stream.emit('restart', 10)  // restart the server in 10 seconds
    })
})

gulp.task('startNotifier', function (cb) {
  const stream = nodemon({
    script: 'botTelegramNotifier/index.js',
    ext: 'js',
    watch: ['botTelegramNotifier'],
    env: {
      LOCALTUNNEL: 0,
      NODE_ENV: config.env,
      DEBUG: `${config.slug}:*`
...
gulp.task('startAll', gulp.parallel('startWebserver', 'watch', (cb) => multiProcess(['startListener'], cb)))
